...
Section PROOF.
  ...
  * whereas internal functions have their frame allocated by the caller *)
  ...
  Variant ex2_5 (T1 T2: Type) (A B C D E : T1 → T2 → Prop) : Prop :=
    Ex2_5 x1 x2 of A x1 x2 & B x1 x2 & C x1 x2 & D x1 x2 & E x1 x2.

  Notation "'exists2_5' x y , p0 & p1 & p2 & p3 & p4" :=
    (ex2_5
       (fun x y => p0)
       (fun x y => p1)
       (fun x y => p2)
  ...
  Let Pfun (ii: instr_info) (k: Sv.t) (s1: estate) (fn: funname) (s2: estate) : Prop :=
    ∀ ls m1 vm1 body ra lret sp callee_saved,
      ...
      exists2_5 m2 vm2,
        ...
        & s2.[vrsp <- Vword (sp_alloc_ra sp ra)] <=[\ ssaved ] vm2
        & preserved_metadata s1 m1 m2
        & match_mem s2 m2.
  ...
  Proof.
    ...
    move=> ls m1 vm1 P Q Mc Xc Dc C hpc hfn.
    ...
    have [m2 vm2 Ei Ki Xi Hi Mi] :=
      Si ls0 m1 vm1 P (lc ++ Q) Mc Xc D C' hpc hfn.
    ...
    + by move: C; rewrite !catA.
    have [m3 vm3] := Sc ls1 m2 vm2 (P ++ li) Q Mi Xi Di Ci erefl hfn.

    rewrite -!catA => E K X H M.
    exists m3 vm3; [ | | exact: X | | exact: M ]; cycle 2.
  ...
  Proof.
    ...
    move=> ls m1 vm1 P Q M X D C hpc hfn.
    have {M X} [m2 vm2 E K X H M] := S _ _ vm1 _ _ M X D C hpc hfn.
    eexists; by eauto.
  Qed.
  ...
  Proof.
    ...
    rewrite /= ok_ds ok_qs.
    move=> ls m1 vm1 P Q M1 X1 D1 C1 hpc hfn.
  ...
  Proof.
    ...
    + rewrite linear_c_nil.
      ...
      have {S} [|m2 vm2 E K2 X2 H2 M2] :=
        S (lnext_pc ls) m1 vm1 P' Q' M1 X1 D' C' _ hfn.
      - by rewrite /P' /= hpc size_rcons.
      have [ b /(match_mem_sem_pexpr M1) ok_e' /value_uinclE ? ] := sem_pexpr_uincl X1 ok_e; subst b.
      exists m2 vm2; [ | exact: K2 | exact: X2 | exact: H2 | exact: M2 ].
    ...
    have {E} [m2 vm2 E K2 X2 H2 M2] :=
      E (setpc ls (size P')) m1 vm1 P' Q' M1 X1 D' C' erefl hfn.
  ...
  Proof.
    ...
    + rewrite linear_c_nil.
      ...
      have {S} [|m2 vm2 E K2 X2 H2 M2] :=
        S (setpc ls (size P')) m1 vm1 P' Q' M1 X1 D' C' _ hfn.
    ...
    have {E} [|m2 vm2 E K2 X2 H2 M2] :=
      E (setpc ls (size P')) m1 vm1 P' (Q' ++ Q) M1 X1 D' C' _ hfn.
    - by rewrite /P' size_rcons.
  ...
  Proof.
    ...
    { (* expression is the “true” literal *)
      ...
      move=> ls m vm P Q M X D C hpc hfn.
      ...
      have {Hc} := Hc (setpc ls (size P0)) m vm P0 Q0 M X.
      case=> //.
      ...
      move => m1 vm1 E1 K1 X1 H1 M1.

      set P1 := P0 ++ lc.
      subst P0 Q0.
      set Q1 := [:: ligoto ] ++ Q.
      have {Hc'} := Hc' (setpc ls (size P1)) m1 vm1 P1 Q1 M1 X1.
      case=> //.
      ...
      move => m2 vm2 E2 K2 X2 H2 M2.

      have {Hw} := Hw ls m2 vm2 P Q M2 X2 D _ hpc hfn.
      case.
      - by rewrite add_align_nil.
      move => m3 vm3 E3 K3 X3 H3 M3.

      exists m3 vm3; [ | | exact: X3 | | exact: M3 ]; cycle 1.
    ...
    { (* second body is empty *)
      ...
      have {Hc} := Hc (setpc ls (size P')) m vm P' Q' M X.
      case=> //.
      - apply: disjoint_labels_cat; last apply: disjoint_labels_cat.
      ...
      have [ b /(match_mem_sem_pexpr M1) {} ok_e /value_uinclE ? ] := sem_pexpr_uincl X1 ok_e; subst b.
      have {Hw} := Hw ls m1 vm1 P Q M1 X1 D _ hpc hfn.
      case.
      - by rewrite add_align_nil.
      move => m3 vm3 E3 K3 X3 H3 M3.
    ...
    have [ b /(match_mem_sem_pexpr M1) {} ok_e /value_uinclE ? ] := sem_pexpr_uincl X1 ok_e; subst b.
    have {Hc'} := Hc' (setpc ls (size P')) m1 vm1 P' Q' M1 X1.
    case=> //.
    - apply: disjoint_labels_cat; last apply: disjoint_labels_cat.
      + apply: disjoint_labels_wL D; lia.
      + subst lcalign. by case: (a).
  ...
  Proof.
    ...
    { (* expression is the “false” literal *)
      ...
      move => Hc.
      move => ls m vm P Q M X D C hpc hfn.
      have {Hc} [ m' vm' E K X' H' M' ] := Hc ls m vm P Q M X D C hpc hfn.
      exists m' vm'; [ exact: E | | exact: X' | exact: H' | exact: M' ].
      apply: eq_exI K; SvD.fsetdec.
    }
    (* arbitrary expression *)
    ...
    { (* second body is empty *)
      ...
      have {Hc} [| m' vm' E  K X' H' M'] :=
        Hc (setpc ls (size prefix)) m vm prefix suffix M X _ C erefl hfn.
      ...
      exists m' vm'; [ | | exact: X' | exact: H' | exact: M' ]; last first.
      - apply: eq_exI K; SvD.fsetdec.
  ...
  Proof.
    ...
    move=> ls m1 vm2 P Q M X D C hpc hfn.
    move: chk_call => /=.
    ...
    have [m' [vm' [hmatch hvm'_rsp heq_vm' hvalue_of hpres_m1_m' h2]]] :
      exists m' vm',
      ...
      * rewrite lnext_pc_setpc -addnS (find_instr_skip C ) //=.
      ...
      + exists m1,  vm2_b.[x <- Vword ptr]; split => //.
        * rewrite Vm.setP_neq; last by case/and3P : ra_sem.
          by rewrite vm2_b_upd Vm.setP_eq /sz_before /rastack_before eq_ra
            vm_truncate_val_eq.
    ...
    move=> m2' vm2' /= h3 heq_vm hsub_vm' hpres hmatch' hk.
    ...
    + move => x x_notin_k.
      ...
      * by subst; rewrite vm2_rsp vm_truncate_val_eq.
      rewrite -heq_vm.
      + apply heq_vm'. move: x_notin_k x_neq_rsp; rewrite hk /ra /=; clear.
 ...
 =>
    ...
    { case: lret ok_lret => // _.
      ...
      { have {ih} := ih fn xH.
        ...
        have {E} [m2 vm2] :=
          E ls0 m1 vm1 [::] [::] M' X' (fun _ _ => erefl) ok_body erefl hfn.
        rewrite /= => E K2 X2 H2 M2.
        eexists m2 _; [ exact: E | | | | exact: mm_free M2 ]; cycle 2.
      ...
      { have {ih} := ih fn xH.
        ...
        have [m2 vm2] :=
          E ls1 m1 vm P Q (mm_alloc M ok_m1') X' D ok_body erefl hfn.
        rewrite /= !size_cat /= addn1.
        move=> {}E K2 X2 H2 M2.
      ...
      { have {ih} := ih fn xH.
        ...
        have [m3 [ok_m3 H3 M3' exec_save_to_stack]] :
          ∃ m3,
        ...
        have [m4 vm4 {}E K4 X4 H4 M4] :=
          E (setpc _ _) m3 vm2 P Q M3 X' D ok_body erefl hfn.
...
+            by rewrite truncate_word_u -wrepr_opp.
    ...
    { case: ra EQ ok_ret_addr X free_ra ok_lret exec_body ih => // -[] // ws // ra EQ ra_well_typed X /andP[] _ ra_notin_k.
      ...
      have {E} [ m2 vm2 E K2 ok_vm2 H2 M2 ] :=
        E ls0 m1 vm1 [:: P] Q (mm_alloc M ok_m1') X1 D ok_body erefl hfn.
      eexists; [ | | | | exact: mm_free M2 ]; cycle 3.
      + move => a [] a_lo a_hi /negbTE nv.
        have A := alloc_stackP ok_m1'.
        have [L H] := ass_above_limit A.
        apply: H2.
    ...
    { 
      ...
      + case: ra EQ X free_ra ok_ret_addr ok_lret => // -[] // ws ra EQ X free_ra ok_ret_addr ok_lret.
        ...
        have {E} [ m2 vm2 E K2 ok_vm2 H2 M2 ] :=
          ...
          (mm_alloc M' ok_m1') X1 D ok_body erefl hfn.
        exists m2 (vm2.[vrsp <- Vword (rsp + wrepr Uptr (wsize_size Uptr))]); last exact: mm_free M2.
        + apply: (lsem_trans3 _ E).
      ...
      exists m2 (vm2.[vrsp <- Vword
         (top_stack (emem s1) - wrepr Uptr (round_ws (sf_align (f_extra fd)) (sf_stk_sz (f_extra fd) + sf_stk_extra_sz (f_extra fd))) + wrepr Uptr (wsize_size Uptr))]); last exact: mm_free M2.
  ...
  Proof.
    ...
    - exact: Export.
    rewrite lp_rspE => lm vm args' vm_rsp M ok_args' args_args' vm_rip safe_registers.
    ...
    case/(_ ls0 lm vm (linear_body liparams p fn fd.(f_extra) fd.(f_body)).2 RAnone None (top_stack m) (map fst fd.(f_extra).(sf_to_save)) M _ _ erefl).
