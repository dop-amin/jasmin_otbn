...
Record h_linearization_params :=
  {
    ...
    hlip_lassign :
      forall
        ...
        -> sem_rexpr s1.(emem) s1.(evm) e = ok (Vword w')
...
Section HLIPARAMS.
  ...
  Lemma spec_lmove lp s1 s2 x ws (w : word ws) (y: var_i) ii fn pc :
    isSome (lmove liparams x ws y)
    -> get_var (evm s1) y = ok (Vword w)
  ...
  Proof.
    rewrite /lmove.
    move=> h hsemy.
    apply: (spec_lassign lp (x := LLvar x) (e := Rexpr (Fvar y)) _ _ _ h hsemy).
...
Section CHECK_SOME.
  ...
  Lemma check_SomeP E e A B c m a u :
    ...
    ∃ b, c a = Some b.
  Proof. by rewrite /check_Some; case: c => // b _; exists b. Qed.
...
Section PROOF.
  ...
  * whereas internal functions have their frame allocated by the caller *)
  ...
  Lemma write_lval_mem_unchanged x v v' s s' t t' :
    ...
    vm_uincl s t →
    match_mem s t →
    forall p, ~~ validw (emem s) p U8 -> read (emem t) p U8 = read (emem t') p U8.
  Proof.
    case: x.
  ...
  Proof.
    move=> ok_s' ok_t' E X M pr _.
    exact: (write_lval_mem_unchanged ok_s' ok_t' E X M).
  Qed.
...
(*     (0 <= max)%Z (* is this needed? *) /\ *)
  ...
  Let Pi (k: Sv.t) (s1: estate) (i: instr) (s2: estate) : Prop :=
    ∀ fn lbl,
     ...
     ∀ m1 vm1 P Q,
       ...
       forall sp, s1.(evm).[ vrsp ]%vmap = ok (pword_of_word sp) ->
       source_mem_split s1 sp ->
       is_stack_max_sub fn sp ->
       ex2_6
       (λ m2 vm2, lsem
                    p'
  ...
  Let Pfun (ii: instr_info) (k: Sv.t) (s1: estate) (fn: funname) (s2: estate) : Prop :=
    ∀ m1 vm1 body ra lret sp callee_saved,
      ...
      is_stack_max fn (top_stack (emem s1)) ->
      ex2_6
      (λ m2 vm2,
  ...
  Proof.
    move => s1 fn lbl _ m1 vm1 P Q M X D C L sp hsp S MAX.
    rewrite !cats0. exists m1 vm1 => //.
    split; first by exact: rt_refl.
    done.
  Qed.
  ...
  Proof.
    ...
    move => m1 vm1 P Q Wc Mc Xc Dc C sp hsp S MAX.
    ...
    have [m2 vm2 [Ei U] Ki Wi Xi Hi Mi ] := Si m1 vm1 P (lc ++ Q) Wc Mc Xc D C' sp hsp S MAX.
    ...
    + by move: C; rewrite !catA.
    have := Sc m2 vm2 (P ++ li) Q Wi Mi Xi Di Ci.
    have hsp': s2.[vrsp]%vmap = ok (pword_of_word sp).
    ...
    have [ m3 vm3 ] := Sc m2 vm2 (P ++ li) Q Wi Mi Xi Di Ci sp hsp' S' MAX.
    rewrite -!catA => -[E U'] K W X H M.
    ...
    + split; first by exact: lsem_trans Ei E.
      move=> pr hnv hpr.
      rewrite (U pr hnv hpr).
      by rewrite (U' pr hnv hpr).
  ...
  Lemma check_rexprsP ii es u :
    ...
    ∀ s vs, sem_pexprs [::] s es = ok vs → sem_rexprs s rs = ok vs.
  Proof.
    case: u; elim: es.
    - by move => _; exists [::].
    move => e es ih.
  ...
  Lemma check_lexprsP ii xs u :
    ...
    ∀ s vs s', write_lvals [::] s xs vs = ok s' → write_lexprs ds vs s = ok s'.
  Proof.
    case: u; elim: xs.
    - by move => _; exists [::].
    move => x xs ih.
  ...
  *)
  ...
  Lemma preserved_metadata_fill_mem m1 m2 m3 m m' ptr bytes :
    fill_mem m1 ptr bytes = ok m ->
    fill_mem m2 ptr bytes = ok m' ->
    preserved_metadata m m' m3 → preserved_metadata m1 m2 m3.
  Proof.
    ...
    have eqv : validw m1 =2 validw m4 by move=> ??; rewrite (write_validw_eq hw1).
  ...
  Lemma preserved_metadata_syscall m1 m2 m3 m m' scs scs' o ves ves' vs vs' :
    List.Forall2 value_uincl ves ves' ->
    exec_syscall_s scs m1 o ves = ok (scs', m, vs) ->
    exec_syscall_s scs m2 o ves' = ok (scs', m', vs') ->
    preserved_metadata m m' m3 → preserved_metadata m1 m2 m3.
  Proof.
    move=> hall hex; have {}:= exec_syscallPs_eq hex hall.
    rewrite /exec_syscall_s; t_xrbindP => -[[scs1 m1'] t1] happ1 [???] -[[scs2 m2'] t2] happ2 [???].
    subst scs1 scs2 m1' m2' vs vs'.
    have h : mk_forall2 (fun o1 o2 => preserved_metadata o1.1.2 o2.1.2 m3 → preserved_metadata m1 m2 m3)
...
>>>>>>> main
    ...
    + rewrite linear_c_nil.
      ...
      - have /= := snot_spec ok_e'.
        ...
        by rewrite /lsem1 /step -(addn0 (size P)) (find_instr_skip C1) /= /eval_instr /li_i (eval_jumpE C1) /to_estate /= ok_e' /= /setpc /= addn0 /P' /Q' size_rcons.
      apply: LSem_step.
      rewrite catA in C'.
...
>>>>>>> main
    ...
    { (* expression is the “true” literal *)
      ...
      - (* align; label *)
        apply:
          (lsem_step_end
             (s2 := {| lfn := fn; lpc := size (P ++ add_align ii a [::]); |}));
          last first.
        + move: C.
    ...
    move => m1 vm1 E1 K1 W1 X1 H1 M1.
    have [ b /(match_mem_sem_pexpr M1) {} ok_e /value_uinclE ? ] := sem_pexpr_uincl X1 ok_e; subst b.
    have {Hc'} := Hc' m1 vm1 (P ++ ι (Lgoto (fn, lbl)) :: add_align ii a [::] ++ [:: ι (Llabel (lbl + 1)) ]) (ι (Llabel lbl) :: lc ++ ι (Lcond f (lbl + 1)) :: Q) W1 M1 X1.
    case.
    - apply: disjoint_labels_cat; last apply: disjoint_labels_cat.
      + apply: disjoint_labels_wL D; lia.
      + by case: (a).
    ...
    - move: C.
      ...
      reflexivity.
    have {} C : is_linear_of fn (P ++ [:: ι (Lgoto (fn, lbl)) ] ++ (add_align ii a [::] ++ ι (Llabel (lbl + 1)) :: lc') ++ [:: ι (Llabel lbl)] ++ lc ++ ι (Lcond f (lbl + 1)) :: Q).
    - move: C; clear.
...
<<<<<<< HEAD
  Lemma stack_frame_allocation_size_pos e :
    (0 <= sf_stk_sz e)%Z ->
    (0 <= sf_stk_extra_sz e)%Z ->
    (0 <= stack_frame_allocation_size e)%Z.
  Proof.
    rewrite /stack_frame_allocation_size.
    have := round_ws_range (sf_align e) (sf_stk_sz e + sf_stk_extra_sz e).
    by lia.
  ...
  Proof.
    move=> hsz hextra.
    rewrite /frame_size.
    have := stack_frame_allocation_size_pos hsz hextra.
    by case: sf_return_address; lia.
  Qed.
...
<<<<<<< HEAD
    ...
    { (* Internal function, return address in register [ra]. *)
      ...
      + exists fd'; first exact: ok_fd'.
        by rewrite /= ra_eq.
(*
      have S': source_mem_split s1 (top_stack (emem s1)). done. apply S. done.
      + move=> pr hvalid; apply /orP.
        move /S : hvalid => /orP [].
        + by move=> ?; left.
        move=> h; right; move: h.
        rewrite /pointer_range !zify.
        move: (MAX _ ok_fd) => /=.
        ...
        + rewrite /stack_frame_allocation_size.
          have := round_ws_range (sf_align (f_extra fd')) (sf_stk_sz (f_extra fd') + sf_stk_extra_sz (f_extra fd')).
          lia.
        rewrite /top wunsigned_sub; first by lia.
        move: (MAX _ ok_fd) => /=.
        
        assert (h := wunsigned_range (top_stack (emem s1))); lia. *)
      have MAX': is_stack_max fn' (top_stack (emem s1)).
      + move=> fd''; rewrite ok_fd' => -[?]; subst fd''.
    ...
    * etransitivity; first exact: z_bound.
    ...
    + move=> pr hvalid; apply /orP.
      ...
      + rewrite /stack_frame_allocation_size.
        have := round_ws_range (sf_align (f_extra fd')) (sf_stk_sz (f_extra fd') + sf_stk_extra_sz (f_extra fd')).
        lia.
      rewrite /sp (top_stack_after_aligned_alloc _ sp_aligned) -/(stack_frame_allocation_size (f_extra fd')).
      rewrite wrepr_opp wunsigned_sub. lia.
      move: (MAX _ ok_fd) => /=.
      assert (h := wunsigned_range (top_stack (emem s1))); lia. *)
    have MAX': is_stack_max fn' (top_stack (emem s1)).
    + move=> fd''; rewrite ok_fd' => -[?]; subst fd''.
    ...
    - split; last first.
      + move=> pr hnm0 hnstack.
        ...
        + move: ok_stk_sz; rewrite !zify /= => -[?[?[? h]]].
          ...
          + move: h; rewrite /frame_size ra_eq. lia.
          move: frame_size_pos => -/(_ _ ltac:(eassumption) ltac:(assumption)). lia.
        ...
        + rewrite /no_overflow zify. rewrite wunsigned_sub.
          ...
          move: (MAX _ ok_fd) => /=.
          move: ok_stk_sz; rewrite !zify /= => -[?[?[??]]].
          move: frame_size_pos => -/(_ _ ltac:(eassumption) ltac:(assumption)).
          have := wunsigned_range sp0; lia.
...
>>>>>>> main
  ...
  Lemma mm_can_write_after_alloc m al sz ioff sz' m' m1 ofs ws (v: word ws) :
    ...
    (0 <= sz')%Z →
    (0 <= max0)%Z ->
    zbetween (sp0 - wrepr Uptr max0) max0 (top_stack m') (sz + sz') ->
    (ws ≤ al)%CMP →
    is_align (wrepr Uptr ofs) ws →
    (sz <= ofs)%Z →
    (ofs + wsize_size ws <= sz + sz')%Z →
  ...
  Proof.
    move => ok_m' sz_pos extra_pos max0_pos hb frame_aligned ofs_aligned ofs_lo ofs_hi M.
    have A := alloc_stackP ok_m'.
    ...
    - case => m2 ok_m2 M2; exists m2; split; [ exact: ok_m2 | | | exact: M2 ].
      ...
      have hb2 : zbetween (top_stack m') (sz + sz') (top_stack m' + wrepr _ ofs) (wsize_size ws).
      + rewrite /zbetween !zify.
        rewrite wunsigned_add //.
        have := A.(ass_above_limit). lia.
      apply (disjoint_zrange_incl_l hb2).
      apply (disjoint_zrange_incl_l hb).
      move: hnstack; rewrite /zbetween /disjoint_zrange !zify wsize8 => hnstack.
      split.
      + rewrite /no_overflow zify wunsigned_sub.
        + have := wunsigned_range sp0. lia.
        have := wunsigned_range sp0. lia.
      + by apply (is_align_no_overflow (sz:=U8)); apply is_align8.
      rewrite {1}wunsigned_sub. lia.
      have := wunsigned_range sp0. lia.
 ...
 =>
    ...
    { case: lret ok_lret => // _.
      ...
      [ | saved_rsp | stack_saved_rsp ] /= ok_save_stack ok_ss ok_to_save exec_body ih ok_fd' wf_to_save S MAX.
      + (* No need to save RSP *)
      { have {ih} := ih fn xH.
        ...
        have hrsp: ((set_RSP p m1' (kill_vars (ra_undef fd {| vtype := sword Uptr; vname := lip_tmp liparams |}) s1)).[vrsp])%vmap =
   ok (pword_of_word (top_stack m1')).
        + by rewrite Fv.setP_eq.
        have S':source_mem_split
   {|
     escs := escs s1;
     emem := m1';
     evm :=
       set_RSP p m1' (kill_vars (ra_undef fd {| vtype := sword Uptr; vname := lip_tmp liparams |}) s1)
   |} (top_stack m1').
        + move=> pr /=.
          rewrite (alloc_stackP ok_m1').(ass_valid).
          rewrite top_stack_preserved.
          rewrite stk_sz_0.
          have ->: between (top_stack (emem s1)) 0 pr U8 = false.
          + apply /idP. rewrite /between !zify wsize8. lia.
        ...
        eexists m2 _; [ split=> //; by exact: E | | exact: W2 | | | exact: mm_free M2 ]; cycle 2.
...
<<<<<<< HEAD
        rewrite -cat_cons.
        case/and4P: ok_stk_sz => /lezP stk_sz_pos /lezP stk_extra_pos /ltzP frame_noof /lezP stk_max_pos.
=======
...
>>>>>>> main
        ...
        have A := alloc_stackP ok_m1'.
<<<<<<< HEAD
        have max0_pos : (0 <= max0)%Z.
        + move: (MAX _ ok_fd) => /=.
          have := frame_size_pos stk_sz_pos stk_extra_pos. lia.
        have hb: zbetween (sp0 - wrepr Uptr max0) max0 (top_stack m1')
                  (sf_stk_sz (f_extra fd) + sf_stk_extra_sz (f_extra fd)).
        + rewrite /zbetween !zify.
          rewrite wunsigned_sub; last first.
          + have := wunsigned_range sp0. lia.
          move: (MAX _ ok_fd) => /=.
          move: stk_max_pos; rewrite /frame_size.
          rewrite EQ.
          move=> stk_max_pos [? [->]].
        ...
        have can_spill := mm_can_write_after_alloc _ ok_m1' stk_sz_pos stk_extra_pos max0_pos hb.
        ...
        have [ m2 [] ok_m2 H2 U M2 ] := can_spill _ _ _ ts aligned_frame rsp_slot_aligned rsp_slot_lo rsp_slot_hi M.
=======
...
<<<<<<< HEAD
          move: wf_to_save ok_to_save m2 H2 U M2.
          elim: (sf_to_save (f_extra fd)) {-2}(sf_stk_sz (f_extra fd))
            => [ sz' _ _ | [x ofs] to_save ih lo /= wf_to_save ok_to_save ] m2 H2 U M2 sz'_le_lo prefix ok_body.
          * by exists m2; split; last (split; first by exact: rt_refl).
=======
...
>>>>>>> main
          ...
          * have := all_disjoint_aligned_between_range ok_to_save. 
          ...
          rewrite topE => acc [] ok_acc Hacc U2 ACC.
          have : preserved_metadata s1 m1 acc.
          * transitivity m2; assumption.
          have : target_mem_unchanged m1 acc.
          * move=> pr hnm0 hnstack. rewrite U //. by apply U2.
          move: ih => /(_ _ _ _ ACC) ih /ih {}ih /ih {}ih.
...
<<<<<<< HEAD
          ...
          split; last first.
          + move=> pr hnm0 hnstack. rewrite U2 //. by apply U3.
=======
   ...
   ok (pword_of_word (top_stack m1')).
       ...
       + move=> pr /=.
         ...
         + move=> /S /orP [].
           ...
           move=> h; right; move: h.
           rewrite /pointer_range !zify.
           have: (wunsigned (top_stack m1') <= wunsigned (top_stack (emem s1)))%Z.
           + rewrite (alloc_stack_top_stack ok_m1').
      ...
      (top_stack (emem s1) + wrepr Uptr (- (sf_stk_sz (f_extra fd) + sf_stk_extra_sz (f_extra fd))))).
       ...
       + move=> fd''; rewrite ok_fd => -[?]; subst fd''.
         ...
         rewrite -(alloc_stack_top_stack ok_m1').
         rewrite wunsigned_add. lia.
         have := A.(ass_above_limit). simpl.
         assert (h1 := wunsigned_range (top_stack (emem s1))).
         assert (h2 := wunsigned_range (top_stack m1')).
         simpl. lia.
      move=> /(_ _ hrsp S' MAX').

        case => m4 vm4 [E U4] K4 W4 X4 H4 M4.
=======
...
>>>>>>> main
    ...
    - (* Internal function, return address in register “ra” *)
    { admit. (* case: ra EQ ok_ret_addr X free_ra ok_lret exec_body ih => // -[] // ws // ra EQ ra_well_typed X /andP[] _ ra_notin_k.
   ...
   ok (pword_of_word (top_stack m1')).
      ...
      + move: ok_stk_sz; rewrite !zify => -[?[?[??]]].
        ...
        + move=> /S /orP.
          ...
          move=> h; right; move: h.
          rewrite /pointer_range !zify.
          rewrite (wunsigned_top_stack_after_aligned_alloc _ _ _ _ ok_m1') //.
          have := stack_frame_allocation_size_pos ltac:(eassumption) ltac:(assumption).
          simpl. lia.
        move=> hb; right; move: hb.
        rewrite /between /zbetween /pointer_range !zify wsize8.
        have: (wunsigned (top_stack m1') + sf_stk_sz (f_extra fd) <= wunsigned sp0)%Z.
        + have /= := (MAX _ ok_fd). rewrite EQ.
          rewrite (wunsigned_top_stack_after_aligned_alloc _ _ _ _ ok_m1') //.
          have: (sf_stk_sz (f_extra fd) <= stack_frame_allocation_size (f_extra fd))%Z.
          + rewrite /stack_frame_allocation_size.
            have := round_ws_range (sf_align (f_extra fd)) (sf_stk_sz (f_extra fd) + sf_stk_extra_sz (f_extra fd)).
            simpl. lia.
          simpl. lia.
        ...
        have MAX': is_stack_max_sub fn (top_stack m1').
        + move=> fd''; rewrite ok_fd => -[?]; subst fd''.
          have := (MAX _ ok_fd). rewrite /frame_size EQ.
          move: ok_stk_sz; rewrite !zify => -[?[?[??]]].
          rewrite (wunsigned_top_stack_after_aligned_alloc _ _ _ _ ok_m1') //=.
          have := stack_frame_allocation_size_pos ltac:(eassumption) ltac:(assumption).
          lia.

      have {E} [ m2 vm2 [E U] K2 W2 ok_vm2 H2 M2 ] := E m1 vm1 [:: P] Q W (mm_alloc M ok_m1') X1 D ok_body _ hrsp S' MAX'.
      eexists; [ | | exact: W2 | | | exact: mm_free M2 ]; cycle 3.
      + move => a [] a_lo a_hi /negbTE nv.
=======
    { 
      ...
      + case: ra EQ X free_ra ok_ret_addr ok_lret => // -[] // ws ra EQ X free_ra ok_ret_addr ok_lret.
        ...
        + apply: lsem_step.
          + rewrite /lsem1 /step /= /find_instr /= ok_fd' /=.
            ...
            + by rewrite truncate_word_u.
            by rewrite /write_lexpr get_varE /= ok_rsp /= !truncate_word_u /= wrepr0 GRing.addr0 ok_m1s.
          apply: lsem_trans; first exact: E.
          apply: LSem_step.
          rewrite catA in ok_body.
...
<<<<<<< HEAD
        ...
        move: ok_stk_sz; rewrite !zify => - [] stk_sz_pos [] stk_extra_pos [] sf_noovf sf_max_pos.
=======
        rewrite /= /set_RSP Fv.setP_eq /= lp_rspE -/vrsp.
        case: vm2.[_]%vmap => // - [] ?? pw_proof /pword_of_word_uincl /= [] ??; subst.
        rewrite (sumbool_of_boolET pw_proof) truncate_word_u /=.
